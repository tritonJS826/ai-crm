...
const suggestionListAtom = atom<SuggestionModel[]>([]);
...
export const addSuggestionsAtom = atom(
  ...
  async (get, set, conversationId: string): Promise<void> => {
    ...
    try {
      const newSuggestions: SuggestionModel[] = await createSuggestions(conversationId);
...
export const loadSuggestionListAtom = atom(
  ...
  async (_get, set, conversationId: string): Promise<void> => {
    ...
    try {
      const suggestionList: SuggestionModel[] = await getSuggestions(conversationId);
